...
| 3 | `No morph map defined for model [Mythos\Core\Identity\Models\User]` au seed | `Relation::enforceMorphMap()` s'applique à **toutes** les relations polymorphiques de l'app, cassant `model_has_roles` de Spatie | Remplacé par `Relation::morphMap()` (non contraignant) |
